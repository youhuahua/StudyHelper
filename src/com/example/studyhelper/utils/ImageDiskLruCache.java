...
public class ImageDiskLruCache {
     ...
     * @return
     */
    public static ImageDiskLruCache openCache(Context context, File cacheDir, long maxByteSize) {
        if (!cacheDir.exists()) {
            cacheDir.mkdir();
        }

        if (cacheDir.isDirectory() && cacheDir.canWrite() && FileUtils.getUsableSpace(cacheDir) > maxByteSize) {
     ...
     * {@link com.example.studyhelper.utils.ImageDiskLruCache#openCache(android.content.Context, java.io.File, long)} which runs some extra
     * checks before creating a DiskLruCache instance.
     * 
     * @param cacheDir
     * @param maxByteSize
     */
    private ImageDiskLruCache(File cacheDir, long maxByteSize) {
        mCacheDir = cacheDir;
    ...
    public void clearCache() {
        ImageDiskLruCache.clearCache(mCacheDir);
    }
     ...
     * be called directly, call {@link com.example.studyhelper.utils.ImageDiskLruCache#clearCache(android.content.Context, String)}
     * or {@link com.example.studyhelper.utils.ImageDiskLruCache#clearCache()} instead.
     * 
     * @param cacheDir
    ...
    public static File getDiskCacheDir(Context context, String uniqueName) {
        ...
        final String cachePath = Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED || !FileUtils.isExternalStorageRemovable() ? FileUtils
     ...
     * {@link com.example.studyhelper.utils.ImageDiskLruCache#setCompressParams(android.graphics.Bitmap.CompressFormat, int)} first to set
